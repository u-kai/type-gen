...
use crate::{
    type_defines::generators::from_json::lang_common::type_statements::type_attr::TypeAttribution,
...
impl TypeAttribution for RustTypeAttributeStore {
    fn get_attr(&self, field_key: &str) -> Option<String> {
        let mut v = Vec::new();
        if let Some(all) = &self.all {
            v.extend(all.clone());
        }
        self.store.get(field_key).map(|attr| {
            v.extend(attr.clone());
        });
        if v.len() > 0 {
...
mod rust_type_attr_test {
    ...
    use crate::type_defines::generators::from_json::lang_common::type_statements::type_attr::TypeAttribution;
